# LEMMA MongoDB Generator Docker Image
The `lemma/mongodb_generator` Docker image bundles LEMMA's
[MongoDB Generator](https://github.com/SeelabFhdo/lemma/tree/master/code%20generators/de.fhdo.lemma.model_processing.code_generation.mongodb)
...
local availability of the `lemma/mongodb_generator` Docker image and guide the
user to install it, if it does not exist already. Furthermore, the script
simplifies the command line invocation of the MongoDB Generator. To this end,
...
/scripts/run_model_processor.py \
    /code generators/de.fhdo.lemma.model_processing.code_generation.mongodb/docker/execution.yaml \
    -b "/home/user" \
...
docker run -i \
    ...
    lemma/mongodb_generator:latest \
