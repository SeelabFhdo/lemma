# LEMMA Container Base Generator Docker Image
The `lemma/container_base_generator` Docker image bundles LEMMA's
[Container Base Generator](https://github.com/SeelabFhdo/lemma/tree/master/code%20generators/de.fhdo.lemma.model_processing.code_generation.container_base)
...
local availability of the `lemma/container_base_generator` Docker image and
guide the user to install it, if it does not exist already. Furthermore, the
script simplifies the command line invocation of the Container Base Generator.
...
/scripts/run_model_processor.py \
    /code generators/de.fhdo.lemma.model_processing.code_generation.container_base/docker/execution.yaml \
    -b "/home/user" \
...
docker run -i \
    ...
    lemma/container_base_generator:latest \
