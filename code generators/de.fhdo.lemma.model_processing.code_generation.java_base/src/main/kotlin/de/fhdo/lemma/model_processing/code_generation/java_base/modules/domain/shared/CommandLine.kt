...
/**
 * Object to represent the basic commandline of the shared library Java generator.
 *
...
internal object CommandLine {
    ...
    /* Commandline option for the target Java package path of the shared library */
    @CommandLine.Option(
        names = ["--package"],
        paramLabel = "JAVA_PACKAGE_NAME",
        description = ["the Java package to be used for the shared library"],
        required = true
    )
    var sharedPackage: String? = null

    /* Commandline option for the target JAR artifact identifier of the shared library */
    var artifactIdentifier: String? = null
        @CommandLine.Option(
            names = ["--artifact_identifier"],
            paramLabel = "ARTIFACT_IDENTIFIER",
            description = [
                "artifact identifier to be used in the generated dependency specification " +
                "(syntax: \"\$GROUP_NAME:\$ARTIFACT_NAME(:\$VERSION)\""
            ],
            required = true
        )
        set(value) {
            try {
                DependencyDescription.fromString(value!!)
            } catch (ex: Exception) {
                throw CommandLine.ParameterException(commandSpec.commandLine(), "Invalid artifact identifier: " +
                    ex.message)
            }
    ...
    /* Commandline option for all intermediate domain models, from which the shared library shall be generated */
    var sharedIntermediateDomainModels: Set<String>? = null
        @CommandLine.Option(
            names = ["--shared_intermediate_domain_model"],
            paramLabel = "INTERMEDIATE_DOMAIN_MODEL_FILE",
            description = ["an intermediate domain model to generate code for a shared library from"],
            required = true
        )
        set(value) {
            value!!.forEach {
                if (!it.asFile().exists())
                    throw CommandLine.ParameterException(commandSpec.commandLine(), "Shared domain model file $it " +
                        "does not exist")
            }
