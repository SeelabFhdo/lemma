...
import de.fhdo.lemma.model_processing.code_generation.springcloud.keycloak.handlers.IntermediateMicroserviceHandler
...
class KeycloakGenerator : AbstractGenlet(BASE_PACKAGE) {
    ...
    override fun onEvent(event: GenletEvent): Pair<Set<GenletGeneratedNode>, Set<GenletGeneratedFileContent>>? {
        if (event.type != GenletEventType.MICROSERVICE_GENERATION_FINISHED)
            return null
        val staticConfigurationClasses = IntermediateMicroserviceHandler.handleMicroserviceGenerationFinished()
        Context.State.reset()
