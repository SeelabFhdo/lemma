package de.fhdo.lemma.model_processing.code_generation.mtls.operation.model_processing_phase
...
import de.fhdo.lemma.model_processing.code_generation.mtls.operation.modul_handler.ModelsContext
...
class LoadAndParseDependentIntermediateModels : AbstractModelProcessingPhase() {
    override fun process(args: Array<String>) {
        ...
        val intermediateInfrastructureNode = intermediateOperationModel.infrastructureNodes.find {
            it.qualifiedInfrastructureTechnologyName == "mTLS.CertificateAuthority"
        }
        if (intermediateInfrastructureNode == null) {
            println("No Certificate-Authority infrastructure was found in this operation model!")
            exitProcess(1)
        }
