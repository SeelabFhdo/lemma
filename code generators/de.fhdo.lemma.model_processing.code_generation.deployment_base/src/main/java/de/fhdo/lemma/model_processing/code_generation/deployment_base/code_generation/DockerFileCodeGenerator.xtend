...
import de.fhdo.lemma.model_processing.code_generation.deployment_base.file.docker.DockerFile
import java.io.File
...
/**
 * Main class of the docker file generation module of the Deployment Base Generator.
 *
...
class DockerFileCodeGenerator extends AbstractCodeGenerationModule {
    ...
    override execute(String[] phaseArguments, String[] moduleArguments) {
        ...
        getContainersWithDeploymentBaseTechnology(model).forEach[createDockerFileForContainer(it)]
    ...
    private def createDockerFileForContainer(IntermediateContainer container) {
        if (container.deployedServices.size > 1)
            throw new PhaseException('''Container «container»: The Deployment Base Generator ''' +
        ...
        else if (appliesAspect(container, DeploymentBaseGenerator.BUILD_MANAGEMENT_TECHNOLOGY_NAME,
            "ArtifactId")) {
