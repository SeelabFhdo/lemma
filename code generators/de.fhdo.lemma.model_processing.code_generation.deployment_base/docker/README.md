# LEMMA Deployment Base Generator Docker Image
The `lemma/deployment_base_generator` Docker image bundles LEMMA's
[Deployment Base Generator](https://github.com/SeelabFhdo/lemma/tree/main/code%20generators/de.fhdo.lemma.model_processing.code_generation.deployment_base)
...
local availability of the `lemma/deployment_base_generator` Docker image and
guide the user to install it, if it does not exist already. Furthermore, the
script simplifies the command line invocation of the Deployment Base Generator.
To this end, the script is executable from the command line with Python 3 and
...
/scripts/run_model_processor.py \
    /code generators/de.fhdo.lemma.model_processing.code_generation.deployment_base/docker/execution.yaml \
    -b "/home/user" \
...
docker run -i \
    ...
    lemma/deployment_base_generator:latest \
