...
/**
 * We use a plugin to specify and resolve Acceleo-specific Eclipse plugin dependencies that are not provided via Maven
...
p2AsMaven {
    val acceleoVersion: String by rootProject.extra
    ...
    group("eclipse-deps", {
        ...
        repo("https://download.eclipse.org/acceleo/updates/releases/$acceleoVersion/")
        iu("org.eclipse.acceleo.engine")
        iu("org.eclipse.acceleo.parser")
...
dependencies {
    ...
    implementation("de.fhdo.ddmm:ocl:$oclVersion:all-dependencies-no-kotlin")
    implementation("info.picocli:picocli:$picocliVersion")
    eclipseDependency("eclipse-deps:org.eclipse.acceleo.engine:+")
    eclipseDependency("eclipse-deps:org.eclipse.acceleo.common:+")
    eclipseDependency("eclipse-deps:org.eclipse.acceleo.model:+")
    eclipseDependency("eclipse-deps:org.eclipse.acceleo.parser:+")
    eclipseDependency("eclipse-deps:org.eclipse.core.runtime:+")
    eclipseDependency("eclipse-deps:org.eclipse.core.contenttype:+")
    eclipseDependency("eclipse-deps:org.eclipse.equinox.common:+")
    eclipseDependency("eclipse-deps:org.eclipse.equinox.registry:+")
    ...
    eclipseDependency("eclipse-deps:com.google.guava:+")
...
/**
 * allDependencies task to create a standalone and runnable fat JAR including all Eclipse-specific and other dependencies
 */
tasks.register("allDependencies", type = Jar::class) {
    ...
    manifest {
        attributes("Main-Class" to "de.fhdo.ddmm.acceleo.parser.StandaloneParserKt")
...
val allDependenciesNoKotlin = task("allDependenciesNoKotlin", type = Jar::class) {
    ...
    from(configurations.compileClasspath.get().filter{ it.exists() && !it.name.startsWith("kotlin") }
        .map { if (it.isDirectory) it else zipTree(it) })
    with(tasks["jar"] as CopySpec)
