...
public class LiveValidationController extends AbstractHandler {
  ...
  public Object execute(final ExecutionEvent event) throws ExecutionException {
    ...
    if (Objects.equal(_id, "de.fhdo.lemma.eclipse.ui.commands.connectToLiveValidationServer")) {
      _matched=true;
      this.connectToLiveValidationServer();
    }
    if (!_matched) {
      if (Objects.equal(_id, "de.fhdo.lemma.eclipse.ui.commands.disconnectFromLiveValidationServer")) {
        _matched=true;
        ServerConnection.instance().disconnect();
      }
    }
    if (!_matched) {
      if (Objects.equal(_id, "de.fhdo.lemma.eclipse.ui.commands.connectionInformation")) {
        _matched=true;
