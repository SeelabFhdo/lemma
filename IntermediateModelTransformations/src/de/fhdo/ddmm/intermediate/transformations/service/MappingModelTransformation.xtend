...
class MappingModelTransformation
    ...
    private def setSourceModelUris(TechnologyMapping mappingModelRoot) {
        ...
        typeMappings.forEach[
            val serviceModelUri = DdmmUtils.removeFileUri(it.serviceModelUri)
            val dataModelUri = DdmmUtils.removeFileUri(it.dataModelUri)

            var absoluteDataModelUri = DdmmUtils.convertToFileUri(
                new File(DdmmUtils.convertToAbsolutePath(dataModelUri, serviceModelUri))
                    .canonicalPath
            )
    ...
    private static final class MappingModelRefinementExecutor {
            ...
            return results
                .findFirst[
                    outputModel.namespaceUri
                        == de.fhdo.ddmm.service.intermediate.IntermediatePackage.eNS_URI &&
        ...
        private static def String buildOutputPathForRefiningTransformation(
            ...
            val refinedModelTargetFolder = DdmmUtils.removeExtension(serviceModel.outputPath)
