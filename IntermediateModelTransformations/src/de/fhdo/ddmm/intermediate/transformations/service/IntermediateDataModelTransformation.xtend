...
class IntermediateDataModelTransformation
    ...
    override prepareInputModel(TransformationModelDescription modelDescription, EObject modelRoot) {
        ...
        if (!DdmmUtils.isFileUri(absoluteInputModelPath))
            dataModel.t_modelUri = DdmmUtils.convertToFileUri(absoluteInputModelPath)
        ...
        dataModel.complexTypeImports.forEach[
            if (!DdmmUtils.isFileUri(importURI))
                importURI = DdmmUtils.convertToFileUri(
                    DdmmUtils.convertToAbsolutePath(importURI, absoluteInputModelPath)
                )
        ]
    }
